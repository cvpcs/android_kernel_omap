...
static int mt9p012_sensor_power_set(struct device* dev, enum v4l2_power power)
{
	...
	case V4L2_POWER_OFF:
		/* Power Down Sequence */
		gpio_set_value(GPIO_MT9P012_RESET, 0);
		mt9p012_set_xclk(0);
...
static int ov5650_sensor_power_set(struct device *dev, enum v4l2_power power)
{
	...
	case V4L2_POWER_OFF:
		...
		gpio_set_value(GPIO_OV5650_RESET, 0);
		gpio_set_value(GPIO_OV5650_STANDBY, 0);

		regulator_poweron = 0;

		gpio_free(GPIO_OV5650_RESET);
		gpio_free(GPIO_OV5650_STANDBY);
	...
	case V4L2_POWER_ON:
		...
		if ((previous_power == V4L2_POWER_OFF) &&
			...
			Some hardware have large filter
			   caps on the VCAM rail. */
			mdelay(10);

			/* Don't try to turn them on again
			if they are already on
			(why do we need this additional flag?) */
			regulator_poweron = 1;
		}

		if (previous_power == V4L2_POWER_OFF) {
			/* Request and configure gpio pins */
			if (gpio_request(GPIO_OV5650_STANDBY,
						"ov5650 camera standby") != 0) {
				pr_err("%s: Failed to request OV5650_STANDBY\n",
						__func__);
				error = -EIO;
				goto out;
			}
			...
			/* Bring camera out of standby */
			gpio_direction_output(GPIO_OV5650_STANDBY, 0);

			if (gpio_request(GPIO_OV5650_RESET,
						"ov5650 camera reset") != 0) {
				pr_err("%s: Failed to request OV5650_RESET\n",
						__func__);
				error = -EIO;
				goto out;
			}
			...
			/* Assert reset */
			gpio_direction_output(GPIO_OV5650_RESET, 0);

			/* Release reset */
			gpio_set_value(GPIO_OV5650_RESET, 1);
...
void __init mapphone_camera_init(void)
{
	...
	if (is_mipi_cam) {
		...
		if (gpio_request(GPIO_MT9P012_RESET,
			...
			pr_err("%s: Failed to request GPIO_MT9P012_RESET\n",
						__func__);

		/* set to output mode, initialize low */

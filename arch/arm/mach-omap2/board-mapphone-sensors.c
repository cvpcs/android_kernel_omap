...
#define OMAP_PWM_PATTERN_SQR_TIME	375
#define OMAP_PWM_PATTERN_OFF_TIME	400
...
static int lvib_pattern_load_next(void)
{
	...
	if (!pwm_pat->this || (pwm_pat->this->type == PWM_PATTERN_OFF))
		switch_on = 1;
...
static int omap_timed_vib_pwm_init(void)
{
	...
	pwm_pat->head = kzalloc(sizeof(struct omap_pwm_reg)
		* (ARRAY_SIZE(omap_pwm_sweep) + 2), GFP_KERNEL);
	if (!pwm_pat->head) {
		printk(KERN_ERR "%s: alloc fail %d\n", __func__, __LINE__);
		kfree(pwm_pat);
		return -1;
	}
	pwm_pat->size = ARRAY_SIZE(omap_pwm_sweep) + 2;
	...
	preg++;
	preg->type = PWM_PATTERN_OFF;
	preg->time = OMAP_PWM_PATTERN_OFF_TIME;
...
static ssize_t isl29030_sysfs_show(struct class *dev, char *buf)
{
	...
	data = gpio_get_value(MAPPHONE_PROX_INT_GPIO);
	str += sprintf(str, "%d\n", data);
	count = (ssize_t) (str - buf);
	return count;
}
...
static void __init mapphone_isl29030_init(void)
{
	...
	else if (class_create_file(class, &class_attr_isl29030)) {
